...
| Every link carries a LABEL — who she sent it to | Without one the list is rows of "Not opened yet, expires 4 Oct" that cannot be told apart, so revoking one is a guess and nobody revokes anything. Production reached TEN links, EIGHT live, FOUR never opened. The label is also what lets an approval say "via the link for Nyall" instead of "Someone approved this" — the weakest provenance in the product, on the decision that gates publication. Staff-only: a client gets NULL from the join and sees the plain wording |
| A minted token is NOT destroyed when the popover closes | It exists on screen exactly once, and a stray Escape used to lose it — the only way back being to mint another, which is where the four never-opened links came from. It stays until "Done — hide it" |
